...
int main(void)
{
	...
	while(k < 10) {
		...
		cudaMalloc(&d_partial_z, threads_per_block*sizeof(double));
		...
		timer.reset();
		for (int n=0; n<10; n++) { 
			dotVec_one<<<256, threads_per_block>>>(d_x, d_y, d_partial_z, N);	
			cudaDeviceSynchronize();
			dotVec_two<<<1, threads_per_block>>>(d_partial_z);
			
			cudaMemcpy(z, d_partial_z, threads_per_block*sizeof(double), cudaMemcpyDeviceToHost);
		}
		printf("%g,%g\n", z[0], 0.1*timer.get());
